...
class OtpRetrieveAPIView(views.APIView):
    """
    Endpoint to retrieve user profile.
    """

    permission_classes = (permissions.AllowAny,)
    serializer_class = serializers.OtpRetrieve

    def post(self, request):
        user_profile = self.get_user_profile(request.data.get('otpkey'))
        if user_profile:
            return Response(status=status.HTTP_200_OK)
    ...
    def get_user_profile(self, otpkey):
        try:
            user_profile = UserProfile.objects.get(otpkey=otpkey)
