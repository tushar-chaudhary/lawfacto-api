...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta(object):
        model = User
        fields = ['username', 'email', 'password', 'password_2', 'first_name', 'last_name', 'phone']
...
class UserLoginSerializer(serializers.ModelSerializer):
    ...
    class Meta(object):
        model = User
        fields = ['email', 'username', 'password', 'phone', 'token']
    ...
    def validate(self, data):
        ...
        if not email and not username and not phone:
            raise serializers.ValidationError("Please enter username or email or phone to login.")
        ...
        if username or email:
            ...
            if user_obj.is_active:
                obj = UserProfile.objects.get(user=user_obj).otp_verified
                if obj:
        ...
        if phone:
            user_obj = Profile.objects.get(phone=phone).user
...
class OtpRetrieve(serializers.ModelSerializer):

    otpkey = serializers.CharField(
        required=True
    )
